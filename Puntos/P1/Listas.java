...
public class Listas {
    ...
    public void l_insertar(Object dato, int pos){
        if((pos>=1 && pos <= l_longitud()) == false){
            System.out.println("No se puede insertar porque la posición es invalida.");
            return;
        }
        if(l_es_Vacia()){
            System.out.println("La lista es vacia, entonces se agrega el dato...");
            agregar(dato);
            return;
        }
